...
func externalLinkingReason(s *modload.State, p *Package) (what string) {
	...
	case "c-shared":
		...
		fallthrough
	case "plugin":
		// mazlink patch: stock Go forces external linking (and therefore
		// the runtime/cgo dependency) for plugin mode. mazlink's patched
		// cmd/link fills the three gaps internally; plugin mode no longer
		// requires cgo. If the user explicitly asks for external link via
		// -ldflags=-linkmode=external, the ldflags loop below still catches
		// that case. See design/GO-PLUGIN-SPIKE.md.
		if cfg.BuildBuildmode == "c-shared" {
			// c-shared is still external-only under mazlink — we only
			// changed the plugin path. c-shared would fall through here
			// from the case above; drop back into the default path.
			return "-buildmode=c-shared"
		}
	}
