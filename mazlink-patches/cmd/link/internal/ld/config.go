...
func mustLinkExternal(ctxt *Link) (res bool, reason string) {
	...
	case BuildModePlugin:
		// mazlink patch: stock Go forces external linking for plugin mode
		// because the external linker (ld) traditionally fills three gaps
		// (.init_array addmoduledata, runtime-symbol export, JUMP_SLOT/TLS
		// relocs). mazlink fills those gaps internally. Fall through; the
		// iscgo check above still forces external when cgo is actually
		// present, so real cgo plugins keep working.
		//
		// See design/GO-PLUGIN-SPIKE.md for the three-gap analysis.
	case BuildModeShared:
