...
func emitPcln(ctxt *Link, s loader.Sym, container loader.Bitmap) bool {
	...
	// mazlink: symbols rewritten to SDYNIMPORT by -dlopen-host-packages
	// remain in ctxt.Textp (AssignTextSymbolOrder uses the object-file
	// kind, not the current SymType). Skip them so their stale funcdata /
	// stkobj aux syms — which still reference now-unreachable runtime data
	// like runtime.gcbits.* — are not walked during pclntab generation.
	if !ctxt.loader.SymType(s).IsText() {
		return false
	}
