...
func (ctxt *Link) addexport() {
	...
	// Mazlink: for BuildModePlugin with internal linker, promote every
	// symbol referenced by go:plugin.exports into ctxt.dynexp so that
	// plugin.Open's dlsym() can find them in .dynsym. Stock Go relies on
	// the external linker to export these; the internal linker has no
	// equivalent path.
	if ctxt.BuildMode == BuildModePlugin && ctxt.LinkMode == LinkInternal {
		ldr := ctxt.loader
		exportsIdx := ldr.Lookup("go:plugin.exports", 0)
		if exportsIdx != 0 {
			seen := make(map[loader.Sym]bool, len(ctxt.dynexp))
			for _, s := range ctxt.dynexp {
				seen[s] = true
			}
			relocs := ldr.Relocs(exportsIdx)
			for i := 0; i < relocs.Count(); i++ {
				target := relocs.At(i).Sym()
				if target == 0 || seen[target] {
					continue
				}
				seen[target] = true
				ctxt.dynexp = append(ctxt.dynexp, target)
			}
		}
	}
	...
	mazlinkNopHostInitTasks(ctxt)
}

// mazlinkNopHostInitTasks marks the runtime package's inittask as already-done
// (state=2) in a plugin build, so the plugin's init pass will not re-run
// runtime.init.N functions. Those functions spawn singleton goroutines
// (forcegchelper, sysmon, bgsweep, bgscavenge, runfinq, gcBgMarkWorker,
// templateThread) which the host has already started; duplicates race on
// shared state and eventually throw ("forcegc: phase error" on amd64,
// scheduler luck delays it on arm64).
//
// The initTask layout (runtime/proc.go) is:
//
//	type initTask struct {
//	    state uint32 // 0 = uninitialized, 1 = in progress, 2 = done
//	    nfns  uint32
//	    // nfns function pointers follow
//	}
//
// Writing 2 at offset 0 causes runtime.doInit1 to see state=2 and return
// without invoking any of the fns. See doInit1 in runtime/proc.go.
//
// This is a pre-dlopen stopgap — Phase 0 of design/MAZARIN-DLOPEN.md. The
// real fix (Phase 2+) makes plugins ship no runtime code at all, so these
// init functions never exist in the plugin's image. Until then we neuter
// the table that drives them.
//
// Only applies to BuildModePlugin + LinkInternal (i.e. mazlink's custom
// plugin shape). Stock Go plugin mode uses the external linker and has a
// different init story.
func mazlinkNopHostInitTasks(ctxt *Link) {
	if ctxt.BuildMode != BuildModePlugin || ctxt.LinkMode != LinkInternal {
		return
	}
	ldr := ctxt.loader
	// Packages whose inittask we neutralize in the plugin image. Keep this
	// list narrow (Phase 0 = runtime only). Grow in lockstep with the
	// host-packages policy list once mazdl lands (see design doc §2).
	hostInittaskSyms := []string{
		"runtime..inittask",
	}
	for _, name := range hostInittaskSyms {
		s := ldr.Lookup(name, 0)
		if s == 0 {
			// Not present in this link — fine (e.g. if a future policy
			// addition isn't referenced by this plugin).
			continue
		}
		// Phase 2: when -dlopen-host-packages is set, runtime..inittask
		// has already been converted to SDYNIMPORT (resolved against the
		// host at load time). In that case the Phase 0 in-place neutering
		// is both unnecessary and impossible (data was cleared). Skip.
		if ldr.SymType(s) == sym.SDYNIMPORT {
			continue
		}
		sb := ldr.MakeSymbolUpdater(s)
		if len(sb.Data()) < 4 {
			// initTask always has at least 8 bytes (state+nfns); a shorter
			// body means something upstream changed. Refuse to patch
			// rather than corrupt.
			Errorf("mazlink: %s data too short (%d bytes); initTask layout changed?",
				name, len(sb.Data()))
			continue
		}
		// Symbol data loaded from an object-file mmap is read-only; mutating
		// it in place faults. MakeWritable copies the bytes into a heap slice
		// and flips the read-only bit when needed.
		sb.MakeWritable()
		sb.SetUint32(ctxt.Arch, 0, 2) // state = done
	}
}
