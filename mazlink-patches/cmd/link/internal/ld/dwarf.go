...
func (d *dwctxt) writelines(unit *sym.CompilationUnit, lineProlog loader.Sym) []loader.Sym {
	...
	for _, s := range unit.Textp {
		...
		// remain in unit.Textp (AssignTextSymbolOrder uses the object-file
		// kind, not the current SymType), but their DWARF aux syms are no
		// longer meaningful — skip them.
		if !d.ldr.SymType(fnSym).IsText() {
			continue
		}
...
func (d *dwctxt) dwarfVisitFunction(fnSym loader.Sym, unit *sym.CompilationUnit) {
	// mazlink: see note in writelines — skip SDYNIMPORT rewrites.
	if !d.ldr.SymType(fnSym).IsText() {
		return
	}
