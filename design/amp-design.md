...
For details on the unified memory pool, page allocation, and bootstrap pre-mapping,
see **[Unified Memory Pool Design](unified-memory-pool.md)**.

Key points:
- Single unified pool serves all page allocations (kernel heap, kernel PT, user frames, user PT)
- Cardinal pre-maps 32MB of the pool for bootstrap page table allocations
- GC is currently disabled due to Go runtime tagged pointer incompatibility with TTBR1 addresses
