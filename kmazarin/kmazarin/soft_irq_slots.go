...
// ============================================================================
// WM Input Completion Ring — Shared Memory IRQ Delivery for HID Input
// ============================================================================
//
// When registered, the IRQ top-half writes HID input events (keyboard, mouse,
// tablet) directly to a userspace-owned page. A mailbox notification wakes the
// window manager shepherd to drain the ring.

// wmInputRingKVA is the kernel VA of the WM's shared input completion ring.
// 0 = not registered (legacy WaitInputEvent path used instead).
var wmInputRingKVA uintptr

// wmInputRingPA is the PA for cleanup (unpin on shepherd death).
var wmInputRingPA uintptr

// wmInputRingOwnerSID is the shepherd that owns the input completion ring.
var wmInputRingOwnerSID int16 = -1

// RegisterInputCompletionRing pins a userspace page and sets it up as the
// shared completion ring for HID input events. Called from ksyscall via linkname.
...
// CleanupInputCompletionRing unpins and releases the input completion ring page
// when the owning shepherd dies. Must be called before CleanupShepherdPages.
func CleanupInputCompletionRing(shepherdID int16) {
	if wmInputRingOwnerSID != shepherdID {
		return
	}
	if wmInputRingPA != 0 {
