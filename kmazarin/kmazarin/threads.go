...
func printEpochStatus() {
	...
	// MAZ-201: delegate reply-gate rejections. Any nonzero value is a caught
	// would-have-been return-value corruption (or leaked retired page); the
	// paired [DLG:*] klog lines carry the per-event detail, but klog is
	...
	badReleases := ksyscall.DelegateBadPageReleases.Load()
	if staleRejects+genRejects+retiredDrops+badReleases > 0 {
		extra += fmt.Sprintf("  replygate: stale=%d gen_mismatch=%d retired_drops=%d bad_release=%d\n",
			staleRejects, genRejects, retiredDrops, badReleases)
	}
