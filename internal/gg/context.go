...
type Context struct {
	...
	// SwapRB, when true, swaps the R and B channels of every color passed
	// to SetColor, SetRGBA255, SetRGB255, SetRGBA, SetRGB, and SetHexColor.
	// This is needed for BGRA framebuffers (e.g., VirtIO GPU).
	SwapRB bool
}
...
// swapRB swaps the R and B channels of a color.
func swapRB(c color.Color) color.Color {
	switch v := c.(type) {
	case color.NRGBA:
		return color.NRGBA{R: v.B, G: v.G, B: v.R, A: v.A}
	case color.RGBA:
		return color.RGBA{R: v.B, G: v.G, B: v.R, A: v.A}
	default:
		r, g, b, a := c.RGBA()
		return color.NRGBA{R: uint8(b >> 8), G: uint8(g >> 8), B: uint8(r >> 8), A: uint8(a >> 8)}
	}
}
...
func (dc *Context) SetFillStyle(pattern Pattern) {
	...
	} else if wrapper, ok := pattern.(*swapRBPattern); ok {
		if fillStyle, ok := wrapper.p.(*solidPattern); ok {
			dc.color = swapRB(fillStyle.color)
		}
	}
...
// swapRBPattern wraps a Pattern and swaps R↔B channels on every ColorAt call.
type swapRBPattern struct {
	p Pattern
}

func (s *swapRBPattern) ColorAt(x, y int) color.Color {
	return swapRB(s.p.ColorAt(x, y))
}
...
// FillRectangle fills a rectangle with the current color using direct pixel
// writes, bypassing the rasterizer. This is significantly faster than
// DrawRectangle+Fill for axis-aligned rectangles. The current color
// (already SwapRB-adjusted via SetColor) is used.
func (dc *Context) FillRectangle(x, y, w, h float64) {
...
func (dc *Context) LoadFontFace(path string, points int64) error {
	face, err := LoadFontFace(path, points)
	if err == nil {
		dc.fontFace = face
		dc.fontHeight = float64(points) * 72 / 96
	}
	return err
}
