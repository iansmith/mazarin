...
// MODIFIED: Added diagnostic prints to debug sizeclass table reads
func lockVerifyMSize() {
	// Print diagnostic info about sizeclass tables BEFORE the check
	print("\n=== lockVerifyMSize DIAGNOSTICS ===\n")

	mSize := unsafe.Sizeof(mPadded{})
	print("mPadded size: ", mSize, " (0x", hex(mSize), ")\n")
	print("gc.MallocHeaderSize: ", gc.MallocHeaderSize, "\n")
	print("gc.SmallSizeMax: ", gc.SmallSizeMax, "\n")
	print("gc.SmallSizeDiv: ", gc.SmallSizeDiv, "\n")
	print("gc.LargeSizeDiv: ", gc.LargeSizeDiv, "\n")
	print("gc.MinSizeForMallocHeader: ", gc.MinSizeForMallocHeader, "\n")
	print("maxSmallSize: ", maxSmallSize, "\n")

	// Replicate the roundupsize calculation with prints
	reqSize := mSize
	print("Initial reqSize: ", reqSize, "\n")

	if reqSize <= maxSmallSize-gc.MallocHeaderSize {
		print("Taking small object path\n")
		// noscan=false for mPadded (it has pointers)
		if reqSize > gc.MinSizeForMallocHeader {
			reqSize += gc.MallocHeaderSize
			print("After adding header: reqSize = ", reqSize, "\n")
		}

		if reqSize <= gc.SmallSizeMax-8 {
			print("Taking SizeToSizeClass8 path\n")
			idx := divRoundUp(reqSize, gc.SmallSizeDiv)
			print("  divRoundUp(", reqSize, ", ", gc.SmallSizeDiv, ") = ", idx, "\n")
			sizeClass := gc.SizeToSizeClass8[idx]
			print("  gc.SizeToSizeClass8[", idx, "] = ", sizeClass, "\n")
			sizeFromClass := gc.SizeClassToSize[sizeClass]
			print("  gc.SizeClassToSize[", sizeClass, "] = ", sizeFromClass, "\n")
		} else {
			print("Taking SizeToSizeClass128 path\n")
			idx := divRoundUp(reqSize-gc.SmallSizeMax, gc.LargeSizeDiv)
			print("  reqSize - gc.SmallSizeMax = ", reqSize-gc.SmallSizeMax, "\n")
			print("  divRoundUp(", reqSize-gc.SmallSizeMax, ", ", gc.LargeSizeDiv, ") = ", idx, "\n")

			// Print the SizeToSizeClass128 table values around the index
			print("  SizeToSizeClass128 table dump:\n")
			start := int(0)
			if int(idx) > 5 {
				start = int(idx) - 5
			}
			end := int(idx) + 10
			if end > len(gc.SizeToSizeClass128) {
				end = len(gc.SizeToSizeClass128)
			}
			for i := start; i < end; i++ {
				val := gc.SizeToSizeClass128[i]
				if i == int(idx) {
					print("    [", i, "] = ", val, " <-- OUR INDEX\n")
				} else {
					print("    [", i, "] = ", val, "\n")
				}
			}

			sizeClass := gc.SizeToSizeClass128[idx]
			print("  gc.SizeToSizeClass128[", idx, "] = ", sizeClass, "\n")
			sizeFromClass := gc.SizeClassToSize[sizeClass]
			print("  gc.SizeClassToSize[", sizeClass, "] = ", sizeFromClass, "\n")

			// CRITICAL DEBUG: Read raw bytes at SizeClassToSize[38] directly
			baseAddr := uintptr(unsafe.Pointer(&gc.SizeClassToSize[0]))
			idx38Addr := baseAddr + 38*2 // uint16 is 2 bytes
			print("  Array base: ", hex(baseAddr), "\n")
			print("  Index 38 calc: ", hex(baseAddr), " + ", 38*2, " = ", hex(idx38Addr), "\n")

			// Read using array syntax first
			arraySyntax := gc.SizeClassToSize[38]
			print("  gc.SizeClassToSize[38] via array: ", arraySyntax, "\n")

			// Read via pointer computed from array element address
			elemPtr := &gc.SizeClassToSize[38]
			elemPtrVal := *elemPtr
			print("  &gc.SizeClassToSize[38] = ", hex(uintptr(unsafe.Pointer(elemPtr))), " value=", elemPtrVal, "\n")

			// Read as uint16 via manual pointer arithmetic
			manualPtr := (*uint16)(unsafe.Pointer(idx38Addr))
			manualVal := *manualPtr
			print("  Manual ptr at ", hex(idx38Addr), " = ", manualVal, "\n")

			// Compare the two addresses
			if uintptr(unsafe.Pointer(elemPtr)) != idx38Addr {
				print("  *** ADDRESS MISMATCH! elemPtr=", hex(uintptr(unsafe.Pointer(elemPtr))), " idx38Addr=", hex(idx38Addr), "\n")
			}

			// Read individual bytes
			byte0Ptr := (*byte)(unsafe.Pointer(idx38Addr))
			byte1Ptr := (*byte)(unsafe.Pointer(idx38Addr + 1))
			byte0 := *byte0Ptr
			byte1 := *byte1Ptr
			assembled := uint16(byte0) | uint16(byte1)<<8
			print("  Byte reads: addr0=", hex(idx38Addr), " val=", hex(uintptr(byte0)), ", addr1=", hex(idx38Addr+1), " val=", hex(uintptr(byte1)), " -> ", assembled, "\n")

			// Read 8 consecutive bytes to see the memory layout
			print("  Memory dump at ", hex(idx38Addr-4), ":\n    ")
			for i := uintptr(0); i < 12; i++ {
				b := *(*byte)(unsafe.Pointer(idx38Addr - 4 + i))
				print(hex(uintptr(b)), " ")
			}
			print("\n")

			// Also check SizeClassToSize[1] to verify table is sane
			idx1Addr := baseAddr + 1*2
			lo1 := *(*byte)(unsafe.Pointer(idx1Addr))
			hi1 := *(*byte)(unsafe.Pointer(idx1Addr + 1))
			raw16_1 := uint16(lo1) | uint16(hi1)<<8
			print("  SizeClassToSize[1] = ", raw16_1, " (expected 8)\n")

			// CRITICAL: Test different access methods
			print("\n  === ACCESS METHOD TEST ===\n")

			// Method 1: Direct index (what the runtime uses)
			method1 := gc.SizeClassToSize[38]
			print("  Method 1 (gc.SizeClassToSize[38]): ", method1, "\n")

			// Method 2: Through a variable
			var scIdx uint8 = 38
			method2 := gc.SizeClassToSize[scIdx]
			print("  Method 2 (via var scIdx=38): ", method2, "\n")

			// Method 3: Through sizeClass variable
			method3 := gc.SizeClassToSize[sizeClass]
			print("  Method 3 (via sizeClass=", sizeClass, "): ", method3, "\n")

			// Method 4: Raw pointer access (computed ourselves)
			method4Ptr := (*uint16)(unsafe.Pointer(baseAddr + 38*2))
			method4 := *method4Ptr
			print("  Method 4 (raw ptr to [38]): ", method4, "\n")

			// Method 5: Array element address
			method5 := &gc.SizeClassToSize[38]
			print("  Method 5 (&gc.SizeClassToSize[38]): addr=", hex(uintptr(unsafe.Pointer(method5))), " val=", *method5, "\n")

			// Check if sizeClass variable got corrupted
			print("  sizeClass variable check: ", sizeClass, " (expected 38)\n")
		}
	} else {
		print("Taking large object path\n")
	}

	// Print array addresses for debugging
	print("\nArray addresses:\n")
	print("  &gc.SizeToSizeClass8[0] = ", hex(uintptr(unsafe.Pointer(&gc.SizeToSizeClass8[0]))), "\n")
	print("  &gc.SizeToSizeClass128[0] = ", hex(uintptr(unsafe.Pointer(&gc.SizeToSizeClass128[0]))), "\n")
	print("  &gc.SizeClassToSize[0] = ", hex(uintptr(unsafe.Pointer(&gc.SizeClassToSize[0]))), "\n")

	// Now do the actual check
	print("\n=== ACTUAL CHECK ===\n")
	size := roundupsize(unsafe.Sizeof(mPadded{}), false) + gc.MallocHeaderSize
	print("Computed size: ", size, " (0x", hex(size), ")\n")
	print("mutexMMask: ", hex(mutexMMask), "\n")
	print("size & mutexMMask: ", hex(size&mutexMMask), "\n")
