...
LINKER_SCRIPT="${1:-src/cardinal/linker.ld}"
...
#
# Strategy:
# 1. Find all constant definitions (BOOT_ADDRESS, DTB_SIZE, ALLOCATION_SIZE_CARDINAL)
...
#   BOOT_ADDRESS = 0x40000000;
#   DTB_SIZE = 0x100000;
#   ALLOCATION_SIZE_CARDINAL = 0xF00000;
#   PAGE_TABLE_SIZE = 0x800000;
#   KMAZARIN_START = PAGE_TABLE_END;
#   PAGE_TABLE_END = PAGE_TABLE_START + PAGE_TABLE_SIZE;
#   PAGE_TABLE_START = CARDINAL_END;
#   CARDINAL_END = CARDINAL_START + ALLOCATION_SIZE_CARDINAL;
#   CARDINAL_START = DTB_END;
#   DTB_END = DTB_START + DTB_SIZE;
#   DTB_START = BOOT_ADDRESS;
#
# So: KMAZARIN_START = BOOT_ADDRESS + DTB_SIZE + ALLOCATION_SIZE_CARDINAL + PAGE_TABLE_SIZE
...
# Extract ALLOCATION_SIZE_CARDINAL (e.g., "ALLOCATION_SIZE_CARDINAL = 0xF00000;")
ALLOCATION_SIZE_CARDINAL=$(grep -E '^\s*ALLOCATION_SIZE_CARDINAL\s*=' "$LINKER_SCRIPT" | \
                          grep -oE '0x[0-9A-Fa-f]+')
...
if [ -z "$BOOT_ADDRESS" ] || [ -z "$DTB_SIZE" ] || [ -z "$ALLOCATION_SIZE_CARDINAL" ] || [ -z "$PAGE_TABLE_SIZE" ]; then
    echo "ERROR: Failed to extract linker script constants" >&2
    echo "  BOOT_ADDRESS=$BOOT_ADDRESS" >&2
    echo "  DTB_SIZE=$DTB_SIZE" >&2
    echo "  ALLOCATION_SIZE_CARDINAL=$ALLOCATION_SIZE_CARDINAL" >&2
    echo "  PAGE_TABLE_SIZE=$PAGE_TABLE_SIZE" >&2
    exit 1
fi

# Calculate KMAZARIN_START = BOOT_ADDRESS + DTB_SIZE + ALLOCATION_SIZE_CARDINAL + PAGE_TABLE_SIZE
# Use printf to do hex arithmetic
KMAZARIN_START=$(printf "0x%X" $(( $BOOT_ADDRESS + $DTB_SIZE + $ALLOCATION_SIZE_CARDINAL + $PAGE_TABLE_SIZE )))
