...
// R_X86_64_RELATIVE entries. A RELATIVE reloc has no symbol — the
...
//
// Other reloc types in .rela.dyn (GLOB_DAT, 64-bit against named syms)
...
func applyRelative(base uintptr, rela []byte) error {
	for off := 0; off+24 <= len(rela); off += 24 {
		...
		if elf.R_X86_64(rType) != elf.R_X86_64_RELATIVE {
			continue
		}
...
// Supported types:
//   - R_X86_64_GLOB_DAT    : *(base + r_offset) = symaddr + addend
//   - R_X86_64_JMP_SLOT    : *(base + r_offset) = symaddr
//   - R_X86_64_64          : *(base + r_offset) = symaddr + addend
//
...
func applySymbolRelocs(base uintptr, rela []byte, syms []elf.Symbol) error {
	for off := 0; off+24 <= len(rela); off += 24 {
		...
		rt := elf.R_X86_64(rType)
		if rt == elf.R_X86_64_RELATIVE {
			continue
		}
