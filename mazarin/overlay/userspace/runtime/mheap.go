...
//
// [MAZ-179 tier-15 probe — NOT FOR MERGE] wrapper enforcing the
// zero-at-handout invariant: a mark-bits chunk that arrives nonzero is
// aliased with live bits (arena recycled under outstanding pointers) or
// was never zeroed — either way it is the observed corruption caught at
// birth. maz179GCBCheckZero lives in maz179_forensic.go.
func newMarkBits(nelems uintptr) *gcBits {
	p := newMarkBits179(nelems)
	if p != nil {
		maz179GCBCheckZero(p, (nelems+63)/64*8)
	}
	return p
}

func newMarkBits179(nelems uintptr) *gcBits {
