...
func (p *_panic) initOpenCodedDefers(fn funcInfo, varp unsafe.Pointer) bool {
	...
	if fn.deferreturn == 0 {
		...
		printstring("BUG-B: initOpenCodedDefers: fn.deferreturn==0\n")
		print("  fn.entry=0x")
		printhex(uint64(fn.entry()))
		gp := getg()
		if gp != nil {
			printstring(" g=0x")
			printhex(uint64(uintptr(unsafe.Pointer(gp))))
			printstring(" stack=[0x")
			printhex(uint64(gp.stack.lo))
			printstring(",0x")
			printhex(uint64(gp.stack.hi))
			printstring("]\n")
		}

		// Scan stack for " failed " corruption pattern.
		if gp != nil {
			for addr := gp.stack.lo; addr < gp.stack.hi; addr += 8 {
				v := *(*uint64)(unsafe.Pointer(addr))
				if v == 0x2064656C69616620 {
					printstring("  FOUND ' failed ' on stack at offset 0x")
					printhex(uint64(addr - gp.stack.lo))
					printstring("\n")
				}
			}
		}
...
// ============================================================================
// Bug-B diagnostic helpers (shepherd-side)
// ============================================================================
// These mirror the kernel's runtime-patches/panic.go diagnostics, adapted to
// use runtime builtins (printstring, printhex) instead of direct UART writes.

// throwPreDiag captures goroutine state before throw() exits.
//
//go:nosplit
func throwPreDiag(s string) {
	printstring("BUG-B: throw(\"")
	printstring(s)
	printstring("\")\n")
	...
	printstring(",0x")
	printhex(uint64(gp.stack.hi))
	printstring("] guard=0x")
	printhex(uint64(gp.stackguard0))
	printstring("\n")
	...
	for addr := lo; addr < hi && found < 16; addr += 8 {
		...
		if v == 0x2064656C69616620 {
			if found == 0 {
				printstring("  STACK SCAN: found ' failed ' at:\n")
			}
			printstring("    offset 0x")
			printhex(uint64(addr - lo))
			printstring(" addr=0x")
			printhex(uint64(addr))
			printstring("\n")
			found++
		}
	}
...
// found (capped at 16). Only prints when hits are found.
...
func ScanCurrentGStackForFailed() uintptr {
	...
	for addr := lo; addr < hi && found < 16; addr += 8 {
		...
		if v == 0x2064656C69616620 {
			if found == 0 {
				printstring("BUG-B: SCAN found ' failed ' on g=0x")
				printhex(uint64(uintptr(unsafe.Pointer(gp))))
				printstring(" stack=[0x")
				printhex(uint64(lo))
				printstring(",0x")
