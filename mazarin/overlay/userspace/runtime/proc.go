...
//   'G' = slow path: got P from pidleget
//   'M' = slow path: no P, going to stopm (M will sleep in futex)
var mazzyExitBreadcrumbs [6]byte // 'O', 'I', 'S', 'G', 'M', '\n'
var mazzyPStatBuf [1]byte        // P status byte for slow path logging

func init() {
	mazzyExitBreadcrumbs[0] = 'O'
	mazzyExitBreadcrumbs[1] = 'I'
	mazzyExitBreadcrumbs[2] = 'S'
	mazzyExitBreadcrumbs[3] = 'G'
	mazzyExitBreadcrumbs[4] = 'M'
	mazzyExitBreadcrumbs[5] = '\n'
}

//go:nosplit
func mazzyBreadcrumb(idx int) {
	write(2, unsafe.Pointer(&mazzyExitBreadcrumbs[idx]), 1)
}
...
func exitsyscall() {
	...
	mazzyBreadcrumb(2) // 'S' — entering slow path
...
func exitsyscallfast(oldp *p) bool {
	...
	if oldp != nil && oldp.status == _Psyscall && atomic.Cas(&oldp.status, _Psyscall, _Pidle) {
		...
		mazzyBreadcrumb(0) // 'O' — got old P back
		return true
	}
	if trace.ok() {
		traceRelease(trace)
	...
	if sched.pidle != 0 {
		...
		if ok {
			mazzyBreadcrumb(1) // 'I' — got idle P
			return true
		}
	}
	return false
...
func exitsyscall0(gp *g) {
	...
	var locked bool
	if pp == nil {
		// MAZZY: log P status while holding sched.lock (accurate snapshot).
		// Status codes: 0=idle, 1=running, 2=syscall, 3=gcstop, 4=dead
		if len(allp) > 0 {
			mazzyPStatBuf[0] = byte(allp[0].status) + '0'
			write(2, unsafe.Pointer(&mazzyPStatBuf[0]), 1)
		}
		globrunqput(gp)
	...
	unlock(&sched.lock)
	if pp != nil {
		mazzyBreadcrumb(3) // 'G' — got P in slow path
		acquirep(pp)
	...
	mazzyBreadcrumb(4) // 'M' — no P, M going to sleep (stopm → futex)
	stopm()
