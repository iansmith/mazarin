...
// PollSoftIRQ polls for soft IRQ events on the given slot.
//
// Uses non-blocking poll + Gosched loop with RawSyscall (no entersyscall).
// The goroutine cooperatively yields the P via Gosched between polls,
// allowing other goroutines to run. Prefer WaitSoftIRQ for event loops
...
func PollSoftIRQ(slot int, buf *hid.SoftIRQReturn) (int, error) {
	for {
		...
		if errno != 0 {
			...
			return 0, errors.New("PollSoftIRQ failed")
		}
...
func TrySoftIRQ(slot int, buf *hid.SoftIRQReturn) (int, error) {
	...
	if errno != 0 {
		...
		return 0, errors.New("TrySoftIRQ failed")
	}
	return int(r1), nil
}
