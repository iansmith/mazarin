...
type fontCache struct {
	...
	tier2      map[rune]*cachedGlyph   // on-demand cache by codepoint
	tier2GID   map[uint32]*cachedGlyph // on-demand cache by GID
}
...
func (c *FontClient) OpenFont(req OpenFontRequest) (FontInfo, error) {
	...
	if c.caches[info.FontID] == nil {
		...
		c.caches[info.FontID] = &fontCache{
			...
			tier2GID:   make(map[uint32]*cachedGlyph),
		}
	}
...
// Returns nil if the font is not loaded. This is needed by the textshape
// package to share the parsed face with HarfBuzz.
func (c *FontClient) Face(fontID int32) *goFont.Face {
	return c.sim.Face(fontID)
}
...
// GlyphByGID returns both metrics and alpha bitmap for a glyph identified
// by its GID (glyph ID). This is used by the textshape package after
// HarfBuzz shaping, which produces GIDs rather than codepoints.
func (c *FontClient) GlyphByGID(fontID int32, gid uint32) (*GlyphInfo, []byte, error) {
	return c.lookupGlyphByGID(fontID, gid)
}

// GlyphInfoByGID returns per-glyph metrics for the given GID.
func (c *FontClient) GlyphInfoByGID(fontID int32, gid uint32) (*GlyphInfo, error) {
	info, _, err := c.lookupGlyphByGID(fontID, gid)
	return info, err
}

// GlyphAlphaByGID returns the alpha-channel bitmap for the given GID.
func (c *FontClient) GlyphAlphaByGID(fontID int32, gid uint32) ([]byte, error) {
	_, alpha, err := c.lookupGlyphByGID(fontID, gid)
	return alpha, err
}
...
// lookupGlyphByGID looks up a glyph by GID. Uses the tier-2 GID cache.
func (c *FontClient) lookupGlyphByGID(fontID int32, gid uint32) (*GlyphInfo, []byte, error) {
	if fontID < 0 || fontID >= maxFonts || c.caches[fontID] == nil {
		return nil, nil, fmt.Errorf("font %d not opened", fontID)
	}
	fc := c.caches[fontID]
	...
	// Miss: render on-demand and cache.
	info, alpha, err := c.sim.RequestGlyphByGID(fontID, gid)
	if err != nil {
		return nil, nil, err
	}
	if info == nil {
