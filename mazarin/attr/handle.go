...
// HandleAny is a type-erased interface for Handle[T] that exposes the slot index.
// Used for dependency registration where the T type parameter doesn't matter.
type HandleAny interface {
	Slot() uint16
	URI() string
}

// Handle[T] wraps a kernel-managed attribute slot with type-safe access.
...
func (h *Handle[T]) URI() string { return h.uri }

// IsDirty reads the dirty flag from the shared page via seqlock.
func (h *Handle[T]) IsDirty() bool {
...
func (h *Handle[T]) Set(v T) {
	if h.kind == flat.AttrKindConstraint {
		panic("attr: cannot Set() on a constraint handle")
	}
	if h.isStr {
		// String values go through the special string syscall.
		var s string
		// Use unsafe to extract string from T (we know T is string for isStr handles).
...
func (h *Handle[T]) seqlockRead() flat.FlatValue {
	node := sharedPR.Node(int16(h.slot))
	for {
		seq := node.SeqCounter
		if seq&1 != 0 {
