// sieve3 is a single-threaded prime sieve program for thread scheduling tests.
// It runs as a separate OS thread/process and prints primes as "3:prime".
package main
...
// It calls WaitKernelAsync which yields to other goroutines when no message
// is pending, keeping the thread running.
func asyncHandler() {
	var bundle sys.KernelAsyncBundle
	for {
		if err := sys.WaitKernelAsync(&bundle); err != nil {
			fmt.Printf("[sieve3] WaitKernelAsync failed: %v, exiting async handler\n", err)
			return
		}
		fmt.Printf("[sieve3] Async: op=%d chan=%d msg=[%d,%d,%d,%d]\n",
