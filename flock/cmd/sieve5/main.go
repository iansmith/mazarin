// sieve5 is a single-threaded prime sieve program for thread scheduling tests.
// It runs as a separate OS thread/process and prints primes as "5:prime".
package main
...
func asyncHandler() {
	...
	for {
		if err := sys.WaitKernelAsync(&bundle); err != nil {
			fmt.Printf("[sieve5] WaitKernelAsync failed: %v, exiting async handler\n", err)
			return
		}
		fmt.Printf("[sieve5] Async: op=%d chan=%d msg=[%d,%d,%d,%d]\n",
...
func main() {
	...
	if err := sys.RegisterAsyncPreempt(); err != nil {
		fmt.Printf("[sieve5] WARNING: RegisterAsyncPreempt failed: %v\n", err)
	}

	fmt.Println("[sieve5] Starting single-threaded prime sieve")
	fmt.Printf("[sieve5] GOMAXPROCS=%d\n", runtime.GOMAXPROCS(0))

	// Start async handler goroutine
	// go asyncHandler()

	// Run sieve starting at 20001, printing primes as "5:prime"
