...
func dtbNameIsDeviceType(p *byte) bool {
	return dtbNameEqualsLiteral(p, &dtbNameDeviceType[0], len(dtbNameDeviceType))
}

func dtbDeviceTypeIsMemory(data *byte, n uint32) bool {
	// Check if device_type value equals "memory" (with NUL terminator)
	if n < uint32(len(dtbDeviceTypeMemory)+1) {
		return false
	}
	for i := 0; i < len(dtbDeviceTypeMemory); i++ {
		b := *(*byte)(unsafe.Pointer(uintptr(unsafe.Pointer(data)) + uintptr(i)))
		if b != dtbDeviceTypeMemory[i] {
			return false
		}
	}
	// Check for NUL terminator
	term := *(*byte)(unsafe.Pointer(uintptr(unsafe.Pointer(data)) + uintptr(len(dtbDeviceTypeMemory))))
	return term == 0
}
...
func getPciEcamFromDTB() (base uintptr, size uintptr, ok bool) {
	...
	if base, size, ok := tryDTBAtBase(dtbBootAddr); ok {
		return base, size, true
	}
	return 0, 0, false
}
...
// getMemoryFromDTB parses the DTB to find the memory node and returns base and size.
// Returns (base, size, ok). If ok is false, memory detection failed.
//
// Uses assembly helpers (asm.GetDtbBootAddr) for DTB address.
func getMemoryFromDTB() (base uintptr, size uintptr, ok bool) {
	// Try DTB pointer from boot first, then QEMU DTB location
	if dtbPtr != 0 {
		if base, size, ok := tryDTBMemory(dtbPtr); ok {
			return base, size, true
		}
	}
	// Use assembly helper to get DTB address
	dtbBootAddr := asm.GetDtbBootAddr()
	return tryDTBMemory(dtbBootAddr)
}

// tryDTBMemory attempts to parse memory info from DTB at given base address.
// Looks for a node with device_type = "memory" and extracts reg property.
func tryDTBMemory(dtbBase uintptr) (base uintptr, size uintptr, ok bool) {
	hdr := (*byte)(unsafe.Pointer(dtbBase))
	magic := be32(hdr)
	if magic != fdtMagic {
		return 0, 0, false
	...
	// Track device_type=memory match and reg property at each depth
	var isMemoryNode [32]bool
	var haveReg [32]bool
	var regAddr [32]uintptr
	var regSize [32]uintptr
	depth := -1
	...
	for iter := 0; iter < 200000; iter++ {
		...
		case fdtBeginNode:
			depth++
			if depth >= len(isMemoryNode) {
				return 0, 0, false
			}
			isMemoryNode[depth] = false
			haveReg[depth] = false
			// Skip node name
			for {
				b := *(*byte)(unsafe.Pointer(p))
		...
		case fdtProp:
			...
			if depth >= 0 {
				...
				if dtbNameIsDeviceType(namePtr) {
					if dtbDeviceTypeIsMemory(val, plen) {
						isMemoryNode[depth] = true
						if haveReg[depth] {
							return regAddr[depth], regSize[depth], true
						}
					}
				...
				// Check for reg property (base, size as 64-bit values)
				if dtbNameIsReg(namePtr) && plen >= 16 {
					addr := be64(val)
					sz := be64((*byte)(unsafe.Pointer(uintptr(unsafe.Pointer(val)) + 8)))
					if sz > 0 {
