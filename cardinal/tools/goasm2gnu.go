...
func main() {
	...
	sectionAlign := flag.Int("align", 0, "Section alignment as power of 2 (e.g., 11 for 2KB)")
	padTo := flag.Int("pad-to", 0, "Pad each TEXT symbol to N bytes with NOPs (e.g., 128 for exception vectors)")
	checkSize := flag.Int("check-size", 0, "Error if any TEXT symbol exceeds N bytes")
	flag.Parse()
	...
	// Step 3: Check sizes if requested
	if *checkSize > 0 {
		for _, sym := range symbols {
			if sym.Section == ".text" && len(sym.Bytes) > *checkSize {
				fmt.Fprintf(os.Stderr, "Error: symbol %s is %d bytes, exceeds limit of %d bytes\n",
					sym.Name, len(sym.Bytes), *checkSize)
				os.Exit(1)
			}
		}
	}

	// Step 4: Pad symbols if requested
	if *padTo > 0 {
		symbols = padSymbols(symbols, *padTo, *verbose)
	}

	// Step 5: Output
	if *elfOutput {
		err = writeELF(*outputFile, symbols, *section, *sectionAlign)
		if err != nil {
...
// padSymbols pads each .text symbol to the specified size with NOP instructions
// ARM64 NOP is 0xD503201F (little-endian: 1F 20 03 D5)
func padSymbols(symbols []Symbol, padTo int, verbose bool) []Symbol {
	nop := []byte{0x1F, 0x20, 0x03, 0xD5} // ARM64 NOP in little-endian
	...
	for i, sym := range symbols {
		...
		currentSize := len(sym.Bytes)
		if currentSize >= padTo {
			// Already at or exceeds target size (check-size should catch exceeds case)
			continue
		}

		// Calculate padding needed
		paddingNeeded := padTo - currentSize

		// Ensure padding is a multiple of 4 bytes (instruction size)
		if paddingNeeded%4 != 0 {
			// Round up to next instruction boundary
			paddingNeeded = ((paddingNeeded + 3) / 4) * 4
		}

		if verbose {
			fmt.Fprintf(os.Stderr, "Padding %s from %d to %d bytes (%d NOPs)\n",
				sym.Name, currentSize, currentSize+paddingNeeded, paddingNeeded/4)
		}

		// Append NOP instructions
		paddedBytes := make([]byte, len(sym.Bytes), padTo)
		copy(paddedBytes, sym.Bytes)
		for j := 0; j < paddingNeeded; j += 4 {
			paddedBytes = append(paddedBytes, nop...)
		}

		result[i].Bytes = paddedBytes
		result[i].Size = len(paddedBytes)
	}
...
// sectionAlign is the alignment as a power of 2 (e.g., 11 for 2KB alignment)
func writeELF(filename string, symbols []Symbol, section string, sectionAlign int) error {
	...
	var textAddralign uint64 = 4 // Default: 4-byte alignment for ARM64 instructions
	if sectionAlign > 0 {
		textAddralign = uint64(1) << uint(sectionAlign) // 2^sectionAlign
	}
