...
// ============================================================================
// Kmazarin Kernel Preemption Support
// ============================================================================
//
// The following code is added for kmazarin kernel preemption support.
// It exposes internal preemption offsets and constants to the kernel
// so it can implement cooperative preemption without calling Go functions
// from IRQ context.

// PreemptOffsets contains the offsets and constants needed for external
// preemption by kmazarin kernel. This struct is filled in at runtime using
// unsafe.Offsetof to avoid hardcoding offsets that may change between Go versions.
type PreemptOffsets struct {
	...
	StackLoOffset uintptr // g.stack.lo
	StackHiOffset uintptr // g.stack.hi
...
// This uses unsafe.Offsetof which is evaluated at compile time for the
// specific Go version, ensuring correct offsets across Go version changes.
//
// Called via go:linkname from kmazarin/kirq package during initialization.
//
//go:linkname GetPreemptOffsets
//go:nosplit
//go:noinline
func GetPreemptOffsets() PreemptOffsets {
	...
	return PreemptOffsets{
		...
		StackLoOffset: unsafe.Offsetof(stackInstance.lo),
		StackHiOffset: unsafe.Offsetof(stackInstance.hi),
