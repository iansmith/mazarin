...
func (u *unwinder) initAt(pc0, sp0, lr0 uintptr, gp *g, flags unwindFlags) {
	...
	// KMAZARIN: If sp0 is 0, bail out to avoid null dereference.
	if sp0 == 0 {
		return
	}
	...
	if frame.fp == 0 {
		...
		if u.flags&unwindJumpStack != 0 && gp == gp.m.g0 && gp.m.curg != nil && gp.m.curg.m == gp.m {
			...
			case abi.FuncID_systemstack:
				...
				u.g.set(gp)
				if gp.sched.sp == 0 {
					println("KMAZARIN: resolveInternal systemstack jump sched.sp=0 goid=", gp.goid, "status=", readgstatus(gp), "sched.pc=", hex(gp.sched.pc))
					if gp.m != nil {
						println("  m.id=", gp.m.id, "m.g0=", hex(uintptr(unsafe.Pointer(gp.m.g0))))
					}
					u.frame = stkframe{}
					return
				}
...
// KMAZARIN: Direct UART MMIO helpers for crash-safe debug output.
// These write directly to the PL011 UART at the kernel virtual address,
// bypassing all Go runtime infrastructure (no allocations, no locks).

//go:nosplit
func kmazarinBreadcrumbByte(b byte) {
	uart := (*uint32)(unsafe.Pointer(uintptr(0xFFFFFFFF09000000)))
	// Wait for UART TX FIFO not full (FR register bit 5)
	fr := (*uint32)(unsafe.Pointer(uintptr(0xFFFFFFFF09000000 + 0x18)))
	for *fr&0x20 != 0 {
	}
	*uart = uint32(b)
}
...
//go:nosplit
func kmazarinBreadcrumbString(s string) {
	for i := 0; i < len(s); i++ {
		kmazarinBreadcrumbByte(s[i])
	}
}

//go:nosplit
func kmazarinBreadcrumbHex(v uint64) {
	const hexChars = "0123456789ABCDEF"
	kmazarinBreadcrumbString("0x")
	for i := 60; i >= 0; i -= 4 {
		kmazarinBreadcrumbByte(hexChars[(v>>uint(i))&0xF])
	}
}
