...
func (c *mcache) refill(spc spanClass) {
	...
	if s != &emptymspan {
		// KMAZARIN PATCH: Skip uncaching if span was never used (allocCount==0).
		// This can happen in exception context where mcache state is not properly initialized.
		...
		if s.allocCount == 0 || s.nelems == 0 {
			...
			// Stale cached spans can occur if GC completed but mcaches weren't flushed.
			// This happens in kmazarin because syscall handlers run outside normal goroutine context.
			// uncacheSpan() already handles both cases properly, so we just need to relax this check.
			sg := mheap_.sweepgen
			// KMAZARIN PATCH: Accept sweepgen+3 (fresh), sweepgen+1 (stale), or 0 (initial/uninitialized).
			// sweepgen=0 can occur in early allocations before GC is fully initialized,
			// or when allocating from exception context where GC state isn't properly propagated.
			if s.sweepgen != sg+3 && s.sweepgen != sg+1 && s.sweepgen != 0 {
				print("runtime: bad sweepgen: span.sweepgen=", s.sweepgen, " mheap_.sweepgen=", sg, " (expected ", sg+3, " or ", sg+1, " or 0)\n")
