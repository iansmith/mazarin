...
func findfunc(pc uintptr) funcInfo {
	...
	fnPtr := uintptr(unsafe.Pointer(&datap.pclntable[funcoff]))
	// Bug-B (MAZ-5 Phase 5b.2): detect if _func pointer resolved into
	// noptrdata (which contains " failed " string constants).
	if datap.noptrdata <= fnPtr && fnPtr < datap.enoptrdata {
		println("BUG-B findfunc: _func pointer in noptrdata")
		println("  search-pc=", hex(pc))
		println("  module=", datap.modulename, " datap=", hex(uintptr(unsafe.Pointer(datap))))
		println("  funcoff=", hex(uintptr(funcoff)), " idx=", hex(uintptr(idx)))
		println("  pcOff=", hex(uintptr(pcOff)), " text=", hex(datap.text))
		println("  noptrdata=[", hex(datap.noptrdata), ",", hex(datap.enoptrdata), ")")
		pclnBase := uintptr(unsafe.Pointer(&datap.pclntable[0]))
		pclnEnd := pclnBase + uintptr(len(datap.pclntable))
		println("  pclntable=[", hex(pclnBase), ",", hex(pclnEnd), ")")
		println("  _func-ptr=", hex(fnPtr))
		throw("findfunc: _func pointer in noptrdata")
	}
	return funcInfo{(*_func)(unsafe.Pointer(fnPtr)), datap}
}
